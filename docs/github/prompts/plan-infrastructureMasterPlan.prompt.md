# Legacy Infrastructure Master Plan Prompt

This file is a docs-side archive of the historical `.github` prompt that
produced an earlier infrastructure master-plan draft.

It is useful as source material, but it is not the canonical roadmap. The
current canonical planning document is
[`../../INFRASTRUCTURE_MASTER_PLAN.md`](../../INFRASTRUCTURE_MASTER_PLAN.md).
When this legacy prompt conflicts with that file, the current master plan wins.

Important reconciliation rule: do not use this prompt to remove the current
master plan's evidence boundary, knowledgebase operations section, Redis
outage failure-mode notes, or anti-overclaim framing.
...
**This plan addresses 11 capability gaps** to transform the stack from a manually-synchronized multi-VPS setup into a fully automated, self-healing, horizontally-scalable personal cloud that anyone can template for their own domain.
...
*Legacy note: the original prompt described itself as the single source of
truth. In this docs archive, that claim has been superseded by
[`../../INFRASTRUCTURE_MASTER_PLAN.md`](../../INFRASTRUCTURE_MASTER_PLAN.md).*
