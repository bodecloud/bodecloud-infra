...
docker node update --label-add node.type=leader wizard-pc
docker node update --label-add storage=true wizard-pc
docker node update --label-add manager_priority=1 wizard-pc
