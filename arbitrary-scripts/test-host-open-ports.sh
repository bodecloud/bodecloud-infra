...
docker exec "$CONTAINER" sh -c '
    ...
    if [ -n "$INSTALL_CMD" ]; then
        ...
        if command -v apk >/dev/null 2>&1; then
            ...
            echo >&2 "${RED}No package manager found to install: $INSTALL_CMD${NC}"
            exit 1
        fi
        ...
        for cmd in $INSTALL_CMD; do
            command -v "$cmd" >/dev/null 2>&1 || { 
                echo >&2 "${RED}Failed to install $cmd${NC}"; 
                exit 1; 
            }
        done
        ...
        echo "${GREEN}Required tools already present.${NC}"
    fi
'
...
for PORT in $(seq "$START_PORT" "$STEP" "$END_PORT"); do
    ...
    if ps -p $HOST_PID > /dev/null; then
        ...
        if timeout 1 nc -z -w 1 "$CONTAINER_IP" "$PORT" >/dev/null 2>&1; then
            echo -e "${GREEN}[$(date +%T)] Internal listener check (BEFORE): successful for port $PORT.${NC}" | tee -a "$RESULTS_FILE"
        else
            echo >&2 "${RED}[$(date +%T)] CRITICAL: Internal listener check (BEFORE) FAILED for port $PORT. Continuing${NC}"
        fi
        ...
        if timeout 1 nc -z -w 1 "$CONTAINER_IP" "$PORT" >/dev/null 2>&1; then
            ...
            echo >&2 "${RED}[$(date +%T)] CRITICAL: Internal listener check (AFTER) FAILED for port $PORT. Continuing${NC}"
        fi
        ...
        echo >&2 "${RED}[$(date +%T)] CRITICAL: Listener process failed to start for port $PORT. Continuing${NC}"
    ...
    if [ -n "$HOST_PID" ]; then
        ...
        {
            # Kill host-side listener process
            kill "$HOST_PID" > /dev/null 2>&1
            wait "$HOST_PID" > /dev/null 2>&1 || true
            # Kill any container-side nc listener on this port
            docker exec "$CONTAINER" pkill -f "nc -l.*-p $PORT" > /dev/null 2>&1
            echo -e "${GREEN}[$(date +%T)] Cleanup completed for port $PORT.${NC}" | tee -a "$RESULTS_FILE"
        } &
    fi
