...
The repo has already escaped the "toy stack" phase.
...
| Local-first serving vs backend-loss survival | If the service is local, keep it local and legible | Traefik local routing, real routers, healthchecks, app surfaces like `whoami`, `wishlist`, `code-server` | Proof that the rescue path survives the failure that made rescue necessary | A local `200` is comforting but says nothing about the actual anti-SPOF claim | Demonstrate local success, then stop the local backend and show preserved route continuity |
...
| HTTP routing logic vs TCP/stateful truth | Stateless HTTP should mature first without lying about harsher lanes | Traefik HTTP and TCP routers already exist for services such as `mongodb` and `redis` | Separate proof rules for transport, authority, promotion, and reconnect semantics | Clean TCP ingress is frequently overread as real HA | Keep HTTP, TCP, and stateful lanes under separate acceptance bars and drills |
| Reachability vs stateful authority | Stateful services should be spoken about much more harshly | Root `mongodb`, root `redis`, `headscale` SQLite, `nuq-postgres`, `rabbitmq`, `litellm-postgres` | Replication, promotion, rediscovery, and writer-authority truth | "It still answers" is not the same as "the write owner stopped being singular" | Per service class, define write authority, failover sequence, promotion rules, and rediscovery behavior |
